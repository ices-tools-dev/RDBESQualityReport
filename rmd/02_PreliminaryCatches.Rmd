...
# TODO: Use vessel flag country instead because it's picking up landings from other countries with CLvesFlagCou
prelC1 <- gather(prelC,  CatchCategory, PrelCWeight, -c(Year, AphiaID, Species.Latin.Name, Area, Country))
prelC1 <- filter(prelC1, Country %in%  unique(CL$CLvesFlagCou))

CL1 <- CL %>%
  group_by(CLarea, CLyear, CLvesFlagCou, CLcatchCat, CLspecCode, LatinName) %>%
...
prelCatches <- left_join(prelC1, CL2, by = c("Year" = "CLyear", "Country" = "CLvesFlagCou", "AphiaID" = "CLspecCode", "Species.Latin.Name" = "LatinName", "Area" = "CLarea", "CatchCategory" = "CLcatchCat"))
