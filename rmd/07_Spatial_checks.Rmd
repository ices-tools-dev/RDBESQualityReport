...
top20_metiers_lv <- CL_DF %>%
  group_by(CLmetier6) %>%
  summarise(lvTot=sum(as.numeric(CLtotOffLanVal), na.rm = TRUE)) %>%
  arrange(desc(lvTot)) %>%
  top_n(20)
...
# For the ices rectangles centroid
# Need to assign the same topMetiers between CL & CE 
# CL weight as variable to determine top 20metiers

top20_metiers_w <- CL_DF %>%
  group_by(CLmetier6) %>%
  summarise(wTot=sum(CLoffWeight, na.rm = TRUE)) %>%
  arrange(desc(wTot)) %>%
  top_n(20)
top20_metiers_w$Top20metiersW <- "X"
CL_DF <- left_join(CL_DF,top20_metiers_w,by="CLmetier6")
CL_DF$topMetiersW <- ifelse(is.na(CL_DF$Top20metiersW),"OTH",CL_DF$CLmetier6)

CE_DF <- left_join(CE_DF,top20_metiers_w,by=c("CEmetier6" = "CLmetier6"))
CE_DF$topMetiersW <- ifelse(is.na(CE_DF$Top20metiersW),"OTH",CE_DF$CEmetier6)
```
...
The top 20 metiers are determined based on the CL official weight. The corresponding metiers are then filtered from the CE table and matched with the CL. 
...
for(country in unique_countries) {
  ...
  for(gear in names(map_combined[[country]])) {
    ...
    print(htmltools::tagList(DT::datatable(CL_without_CE[[country]][[gear]], filter = 'top', rownames = FALSE)))
