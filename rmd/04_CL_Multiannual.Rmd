...
rcgq1 <- list()
for(i in unique(CLws$Code)){
rcgq1[[i]] <- ggplot(filter(CLws, Code %in% i), aes(CLarea, CLsciWeight, fill = CLyear)) +
  geom_bar(stat = "identity", position = "dodge") +
  facet_grid(CLquar~.) +
  theme_bw() +
  theme(text = element_text(size = 20),
...
```{r results = "asis", eval = doNextChunkLan,fig.dim= c(12, 8)}

for(i in unique(names(rcgq1))){
    # The tabset level here has to be lower than the
    # parent level (ie, parent is 2, so here you have to use 3)
    catHeader(names(rcgq1[i]), 6)
...
```{r results = "asis",eval = doNextChunkBMS, fig.dim= c(12, 8)}
...
```{r results = "asis", eval = doNextChunkDis, fig.dim= c(12, 8)}
...
ggplot(data = CLvs,
       aes(axis1 = CLvesFlagCou, axis2 = CLlanCou, y = Weight)) +
  scale_x_discrete(limits = c("Vessel Flag Country", "Landing Country")) +
  ylab("Weight (t)") +
  geom_alluvium(aes(fill = WeightSource)) +
  geom_stratum() +
...
top20_CLSpecies <- CLlan %>%
  group_by(LatinName) %>%
  summarise(lvTot=sum(CLsciWeight, na.rm = TRUE)) %>%
  arrange(desc(lvTot)) %>%
  top_n(20)
...
top20_CLSpecies <- CLlan %>%
  group_by(LatinName) %>%
  summarise(lvTot=sum(CLtotOffLanVal, na.rm=TRUE)) %>%
  arrange(desc(lvTot)) %>%
  top_n(20)
top20_CLSpecies$Top20CLSpecies <- "X"
CL1 <- left_join(CLlan,top20_CLSpecies,by="LatinName")
...
CLLoc <- CLvalS %>%
  group_by(topCLSpecies) %>%
  mutate(TotV = sum(CLtotOffLanVal, na.rm = TRUE))
