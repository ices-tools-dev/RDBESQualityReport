...
```{r}
# Transform kg to t 
CL$CLoffWeight <- CL$CLoffWeight/1000
CL$CLsciWeight <- CL$CLsciWeight/1000
...
CLws <- CL %>%
  group_by(CLyear, CLdTypSciWeig, CLdSouSciWeig,  CLarea) %>%
  summarise(CLsciWeight = sum(CLsciWeight, na.rm = TRUE))

ggplot(CLws, aes(CLarea, CLsciWeight, fill = CLyear)) +
  geom_bar(stat = "identity", position = "dodge") +
...
CLwn <- CL %>%
  group_by(CLyear, CLdTypSciWeig, CLdSouSciWeig,  CLsampScheme, CLarea) %>%
  summarise(CLsciWeight = sum(CLsciWeight, na.rm = TRUE))
```
...
CLw <- CL %>%
  ...
  group_by(CLyear, CLquar, CLarea) %>%
  summarise(CLoffWeight = sum(CLoffWeight, na.rm = TRUE),
            CLsciWeight = sum(CLsciWeight, na.rm = TRUE))
CLw <- gather(CLw, WeightSource, Weight, -c(CLyear, CLarea, CLquar))
...
CLlrd <- CL %>%
  ...
  group_by(CLyear, CLregDisCat, CLarea) %>%
  summarise(CLoffWeight = sum(CLoffWeight, na.rm = TRUE),
            CLsciWeight = sum(CLsciWeight, na.rm = TRUE))
CLlrd <- gather(CLlrd, WeightSource, Weight, -c(CLyear, CLarea, CLregDisCat))

ggplot(CLlrd, aes(CLarea, Weight, fill = CLyear)) +
  geom_bar(stat = "identity", position = "dodge") +
  facet_grid(CLregDisCat~.) +
  theme_bw() +
  theme(text = element_text(size = 20),
        axis.text.x = element_text(angle = 90, hjust = 1)) 
```
