...
CLvl <- CL %>%
  ...
  summarise(CLoffWeight = sum(CLoffWeight, na.rm = TRUE),
            CLsciWeight = sum(CLsciWeight, na.rm = TRUE))
datatable(CLvl, caption = "Total weight by vessel length category by year")
```
...
```{r}
CLlan <- filter(CL, CLcatchCat %in% "Lan")
CLlan$CLlandingsValue <- as.numeric(CLlan$CLlandingsValue)
top20_CLSpecies <- CLlan %>%
  group_by(LatinName) %>%
  summarise(lvTot=sum(CLlandingsValue)) %>%
  arrange(desc(lvTot)) %>%
  top_n(20)
top20_CLSpecies$Top20CLSpecies <- "X"
CL1 <- left_join(CLlan,top20_CLSpecies,by="LatinName")
...
CLvalS <- CL1 %>%
  group_by(topCLSpecies, CLyear) %>%
  summarise(CLlandingsValue = sum(CLlandingsValue, na.rm = TRUE)) 

CLLoc <- CLvalS %>%
  group_by(topCLSpecies) %>%
  mutate(TotV = sum(CLlandingsValue))


ggplot(CLLoc, aes(reorder(topCLSpecies, -TotV), CLlandingsValue, fill = CLyear)) +
  geom_bar(position="stack", stat="identity") +
  theme_bw() +
  theme(text = element_text(size = 20), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1), legend.key.size = unit(2, 'cm'))+
  xlab("topCLSpecies") +
  ylab("CLlandingsValue")
```
...
for(i in sort(unique(CL2$topCLSpecies))){
  val[[i]] <- ggplot(filter(CL2, topCLSpecies %in% i), aes(CLlandingsValue, CLsciWeight, color = CLyear)) +
    geom_point() +
    theme_bw()+
  theme(text = element_text(size = 20), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1), legend.key.size = unit(2, 'cm'))
}
